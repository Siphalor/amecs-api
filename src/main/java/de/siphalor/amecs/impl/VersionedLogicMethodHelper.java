...
public class VersionedLogicMethodHelper {
	...
	public static List<MethodFieldAndName> getMethodFieldAndNamesForClass(Class<?> clazz) {
		...
		for (Field f : clazz.getDeclaredFields()) {
			...
			try {
				...
				AmecsAPI.log(Level.WARN, "Found logic method search method in class \"" + clazz.getName() + "\" but no associated logic method name prefix");
				e.printStackTrace();
				continue;
			}
		}
	...
	public static void initLogicMethod(Class<?> clazz, MethodFieldAndName fieldAndName) {
		...
		if (logicMethod == null) {
			throw new IllegalStateException("No \"" + fieldAndName.logicMethodNamePrefix + "\" method available for minecraft Version: " + AmecsAPI.SEMANTIC_MINECRAFT_VERSION.getFriendlyString());
		}
	...
	public static class ReflectionExceptionProxiedMethod {
		...
		public Object invoke(Object instance, Object... args) {
			try {
				...
				AmecsAPI.log(Level.ERROR, "Error while executing: \"" + method.getName() + "\" in class: \"" + method.getDeclaringClass().getName() + "\"");
				e.printStackTrace();
			}
			return null;
		}
